import { HttpIngestSchema, HttpIntentSchema, HttpQuerySchema, PlacementCandidateSchema } from "@lastseen/shared";
import type { HttpIntent, HttpQuery, PlacementCandidate } from "@lastseen/shared";
...
export function parseIntent(json: unknown): Parsed<HttpIntent> {
  const r = HttpIntentSchema.safeParse(json);
  return r.success ? { ok: true, value: r.data } : { ok: false, status: 400, error: `invalid intent: ${issues(r.error)}` };
}
