import { AgentStepSchema, CropDescriptionSchema, IntentResultSchema, PlacementResultSchema, VerifyResultSchema } from "@lastseen/shared";
import type { AgentStep, CropDescription, IntentResult, PlacementResult, VerifyResult } from "@lastseen/shared";
import type { ZodType } from "zod";
import { BudgetGuard, DEFAULT_PRICING, estimateCad } from "./budget";
import type { Pricing } from "./budget";
import { CROP_SYSTEM, PLACEMENT_SYSTEM, STEP_SYSTEM, VERIFY_SYSTEM, intentSystem, renderHistory } from "./prompts";
...
export class HttpOmni implements OmniClient {
  ...
  understandIntent(audio: AudioClip | null, text: string | undefined, targets: IntentTarget[]): Promise<OmniResult<IntentResult>> {
    const parts: Part[] = [];
    if (text) parts.push({ type: "text", text: `WEARER SAID (typed): ${text}` });
    if (audio && this.capabilities.audioInput) parts.push(this.audioPart(audio));
    if (!parts.length) parts.push({ type: "text", text: "(nothing was heard)" });
    return this.jsonCall(intentSystem(targets), parts, IntentResultSchema);
  }
