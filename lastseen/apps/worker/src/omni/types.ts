import type { AgentStep, CropDescription, Detection, IntentResult, PlacementResult, VerifyResult } from "@lastseen/shared";
...
/** An object a client can be pointed to by name: what its detector calls it, and the ways a wearer might say it. */
...
export interface OmniClient {
  ...
  /** does the wearer's utterance (audio, or text) ask to be pointed to one of `targets`? Stateless: no memory involved */
