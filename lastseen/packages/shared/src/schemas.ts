...
/**
 * POST /api/intent body: one spoken (or typed) utterance. Stateless: no memory, no device. Used by a client that already knows where
 * the object is (an on-device detector) and only needs to know whether the wearer asked for it.
 */
export const HttpIntentSchema = z
  .object({
    text: z.string().min(1).max(500).optional(),
    audioB64: z.string().optional(),
    mime: z.string().optional(),
  ...
  .refine((q) => Boolean(q.text) || Boolean(q.audioB64), { message: "send text or audioB64" });
export type HttpIntent = z.infer<typeof HttpIntentSchema>;

/** What OMNI made of one utterance, before the Worker validates `wants` against the known targets. */
...
export interface HttpIntentReply {
  heard: string;
  ...
  /** why OMNI could not answer, when source is "keywords" */
  note?: string;
}
