...
public final class MediaPipeDetector {
    ...
    private static String toJson(ObjectDetectorResult result, int width, int height, String targetClass) {
        try {
            ...
            for (Detection detection : result.detections()) {
                ...
                int x1 = Math.max(0, Math.min(width, Math.round(box.left)));
                int y1 = Math.max(0, Math.min(height, Math.round(box.top)));
                int x2 = Math.max(x1, Math.min(width, Math.round(box.right)));
                int y2 = Math.max(y1, Math.min(height, Math.round(box.bottom)));
