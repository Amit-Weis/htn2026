...
The reusable Android library accepts packed RGBA8888 bytes (`width * height * 4`) in top-left-origin image order and returns JSON. The target must use the embedded model label, such as `cell phone`, not `phone`.
...
2. Run `python forgetmenot_cv/download_model.py --android`.
...
The XREAL frame source produces an aligned `cell phone` box using the shared Android detector.
...
- The Android library is source-integrated and has not yet been compiled through Unity in this environment.
